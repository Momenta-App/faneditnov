...
export async function GET(
  ...
  try {
    ...
    // First, get all video IDs for this hashtag
    ...
    if (hashtagError) {
      console.error('Error fetching hashtag videos:', hashtagError);
    ...
    const transformedData = data?.map((video: any) => ({
      ...
      platform: video.platform || (video.url || video.video_url ? detectPlatform(video.url || video.video_url) : 'unknown'), // Platform: tiktok, instagram, youtube, or unknown
      creator: {
    ...
    console.error('Error in hashtag videos API:', error);
    return NextResponse.json(

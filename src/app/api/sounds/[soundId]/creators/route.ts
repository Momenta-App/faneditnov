...
export async function GET(
  ...
  try {
    ...
    const { data, error } = await supabaseAdmin.rpc('get_sound_creators', {
      p_sound_id: soundId
    });

    if (error) {
      console.error('Error calling get_sound_creators function:', error);
      
      // Fallback to manual query if function doesn't exist
      console.log('Falling back to manual aggregation...');
      return await getSoundCreatorsFallback(soundId);
    }
    ...
    console.error('Error in sound creators API:', error);
...
async function getSoundCreatorsFallback(soundId: string) {
  try {
    // Get all video IDs for this sound

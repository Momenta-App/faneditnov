    ...
    >
      <nav className="container-base max-w-[1440px] mx-auto" aria-label="Main">
        <div className="flex h-16 items-center justify-between gap-6">
          ...
          {/* Desktop Navigation */}
          <div className="hidden lg:flex lg:items-center lg:gap-3 grow justify-center">
            {navigation.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className={`relative inline-flex items-center justify-center px-6 py-3 rounded-lg text-sm font-medium transition-all focus-ring min-h-[44px] ${
                  isActive(item.href)
              ...
              >
                ...
                <span 
                  className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 w-1 h-1 rounded-full transition-all ${
                    isActive(item.href) ? 'bg-[var(--color-primary)]' : 'opacity-0'
                  }`}
                ></span>
              </Link>
            ))}

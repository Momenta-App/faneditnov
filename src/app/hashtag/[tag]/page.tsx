...
import { useHashtag, useHashtagVideos, useHashtagCreators } from '../../hooks/useData';
...
export default function HashtagPage() {
  ...
  const { data: hashtag, loading: loadingHashtag } = useHashtag(decodedTag);
  const { data: filteredVideos, loading: loadingVideos } = useHashtagVideos(
    decodedTag,
    searchQuery,
  ...
  // Show loading state while fetching hashtag
  if (loadingHashtag) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--color-background)' }}>
        <div className="text-center">
          ...
          <p className="mt-4" style={{ color: 'var(--color-text-muted)' }}>Loading hashtag...</p>
        </div>
      </div>
    );
  }

  // Show not found if hashtag doesn't exist and has no videos
  if (!hashtag || (hashtag.videos === 0 && !loadingVideos)) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--color-background)' }}>
        <div className="text-center">
          <h2 className="text-2xl font-bold mb-4" style={{ color: 'var(--color-text-primary)' }}>Hashtag Not Found</h2>
          <button
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--color-background)' }}>
      ...
      <BrandAccountHeader
        title={`#${hashtag?.name || decodedTag}`}
        stats={[
          { value: formatNumber(hashtag?.views || 0), label: 'Total Views' },
          { value: formatNumber(hashtag?.videos || 0), label: 'Videos' },
          { value: formatNumber(hashtag?.creators || 0), label: 'Creators' },
          { value: formatNumber(hashtag?.impact || 0), label: 'Impact Score' },
        ]}
              ...
              {loadingVideos ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
                  {filteredVideos.map((video) => (
                    <VideoCard key={video.id} video={video} hideLikes={true} />
                  ))}
                </div>
              ) : (
                <NoVideosEmptyState
